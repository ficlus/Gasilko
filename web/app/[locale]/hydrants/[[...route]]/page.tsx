...
export default async function Hydrants({ params, searchParams }: { params: Promise<{ locale: string; route?: string[] }>; searchParams: Promise<{ org?: string | string[] }> }) {
  ...
  return <main className="registry-main"><h1>{t.hTitle}</h1><Registry key={`${locale}/${route.join('/')}/${org ?? ''}`} locale={locale} mode={mode} id={id} org={org}/><nav><Link href={`/${locale}/account`}>{t.backAccount}</Link></nav><SessionControls locale={locale}/></main>;
}
